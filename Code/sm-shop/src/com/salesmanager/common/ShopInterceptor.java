...
public abstract class ShopInterceptor extends SalesManagerInterceptor {
	...
	public String baseIntercept(ActionInvocation invoke,
		...
		// Custom Portlet Map (position, List<PortletConfiguration>
		...
		if (vo != null && vo.getMerchantConfigurationList() != null
			...
			/**
			 * Labels required for the sore Shipping Section Page Titles
			 * only portlets
			 */
			...
			if (dynamicLabels == null) {
				...
				if (!missed) {
					...
					dynamicLabels = rservice.getDynamicLabels(mStore
							.getMerchantId(), LabelConstants.STORE_FRONT_CUSTOM_PORTLETS, locale);
		 ...
		 * TEMPLATE
		 */
		req.setAttribute("templateId", mStore.getTemplateModule());
