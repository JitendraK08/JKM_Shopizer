...
<s:if test="categories!=null && categories.size>0" >
	...
	<s:iterator value="categories" status="count">
			...
			<s:if test="#session.subCategory!=null && #session.subCategory.categoryId==id.categoryId">
				<a href="<sm:url scheme="http" namespace="/category" action="${categoryDescription.url}" />" class="sub-link href-selected">
			</s:if>
			<s:else>
				<a href="<sm:url scheme="http" namespace="/category" action="${categoryDescription.url}" />" class="sub-link">
			</s:else>
