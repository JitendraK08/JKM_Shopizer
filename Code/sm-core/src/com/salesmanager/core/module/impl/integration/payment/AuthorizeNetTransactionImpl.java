...
public class AuthorizeNetTransactionImpl extends
	 ...
	 */
	/*
	 * public void processTransaction(CoreModuleService
	 * serviceDefinition,PaymentMethod paymentMethod,Order order)throws
	 ...
	 * ConfigurationRequest configrequest = new
	 * ConfigurationRequest(order.getMerchantId
	 * (),PaymentConstants.MODULE_PAYMENT_GATEWAY +
	 * PaymentConstants.PAYMENT_AUTHORIZENETNAME); MerchantService service =
	 * (MerchantService
	 * )ServiceFactory.getService(ServiceFactory.MerchantService); vo =
	 * service.getConfiguration(configrequest);
	 * 
	 * 
	 * if(vo==null) { TransactionException te = new
	 * TransactionException("AuthorizeNet Payment Gateway not configured for " +
	 * vo.getConfiguration("paymentmethod")+
	 * ", cannot retreive credentials for merchantid " + order.getMerchantId());
	 * throw te; }
	 * 
	 * IntegrationKeys ik = (IntegrationKeys)vo.getConfiguration("keys");
	 * IntegrationProperties props =
	 * (IntegrationProperties)vo.getConfiguration("properties");
	 * 
	 * 
	 * 
	 * if(order.getMerchantId()==0) { throw new
	 * TransactionException("merchantId is not set in Order"); }
	 * 
	 ...
	 * TransactionException("MerchantStore is null for merchantId " +
	 * order.getMerchantId()); }
	 * 
	 * int originCountryId = store.getCountry();
	 * 
	 * //determine which kind of transaction needs to be processed //if
	 * transaction type = 1 = pre-authorization //if transaction type = 2 =
	 * authorizeAndCapture if(props!=null &&
	 * !StringUtils.isBlank(props.getProperties1())) {
	 * if(props.getProperties1().
	 * equals(String.valueOf(PaymentConstants.PREAUTH))) {
	 * this.authorizeTransaction(originCountryId, order); } else
	 * {//preAuthAndCapture this.authorizeAndCapture(originCountryId, order); }
	 * } else { this.authorizeAndCapture(originCountryId, order); }
	 * 
	 * 
	 * 
	 * } catch (Exception e) { if(e instanceof TransactionException) { throw
	 * (TransactionException)e; } else { throw new TransactionException(e); } }
	 * 
	 * 
	 * 
	 * }
...
/*			// Get information on the gateway
			...
			CoreModuleService cis = ModuleManagerImpl.getModuleServiceByCode(
					countrycode, PaymentConstants.PAYMENT_AUTHORIZENETNAME, 1);// 1
																				// is
																				// type
																				// credit
																				// card
			...
			/*
			 * 
			 * // open secure connection URL anurl = new URL(protocol + "://" +
			 * host + ":" + port + url);
			 * 
			 ...
			 * 
			 * // POST the data in the string buffer DataOutputStream out = new
			 * DataOutputStream( connection.getOutputStream() );
			 * out.write(sb.toString().getBytes()); out.flush(); out.close();
			 * 
			 * // process and read the gateway response BufferedReader in = new
			 * BufferedReader(new
			 * InputStreamReader(connection.getInputStream())); String line;
			 * line = in.readLine(); in.close(); // no more data
			 */
...
/*			// 1)Retreive capturable transaction
			...
			if (!trx.getName()
					.equals(PaymentConstants.PAYMENT_AUTHORIZENETNAME)) {
				TransactionException pe = new TransactionException(
						"Cannot use this payment module to process transaction "
								+ order.getOrderId());
				pe.setErrorcode("03");
				throw pe;
			}
			...
			if (vo == null) {
				...
				// Retreive transaction configuration
			...
			if (vo.getConfiguration("paymentmethod") == null
					|| !vo.getConfiguration("paymentmethod").equals(
							PaymentConstants.PAYMENT_AUTHORIZENETNAME)) {
				TransactionException te = new TransactionException(
						"AuthorizeNet Payment Gateway not configured for "
								+ vo.getConfiguration("paymentmethod")
...
*/
			...
			if (result != 200) {
				...
				// TransactionException("Psigate Gateway error ");
				// te.setErrorcode("01");
				// throw te;
				throw new Exception("Communication Error with Authorizenet "
						+ protocol + "://" + host + ":" + port + url);
	...
	public GatewayTransactionVO refundTransaction(IntegrationKeys keys, IntegrationProperties props,
		...
		/*	// 1)Retreive refundable transaction
			PaymentService paymentservice = new PaymentService();
			GatewayTransactionVO trx = paymentservice
			...
			ConfigurationRequest configrequest = new ConfigurationRequest(order
					.getMerchantId(), PaymentConstants.PAYMENT_AUTHORIZENETNAME);
			MerchantService service = (MerchantService) ServiceFactory
					.getService(ServiceFactory.MerchantService);
