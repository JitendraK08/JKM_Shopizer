...
/*			String countrycode = CountryUtil.getCountryIsoCodeById(store
			...
			CoreModuleService cis = ModuleManagerImpl.getModuleServiceByCode(
					countrycode, PaymentConstants.PAYMENT_MONERIS, 1);// 1 is
																		// type
																		// credit
																		// card
			...
			if (cis == null) {
				log.error("Central integration services not configured for "
						+ PaymentConstants.PAYMENT_MONERIS + " and country id "
						+ store.getCountry());
				throw new Exception(
			...
			// String host = cis.getCoreModuleServiceDevProtocol() + "://" +
			// cis.getCoreModuleServiceDevDomain() + ":" +
			// cis.getCoreModuleServiceDevPort();
			...
			if (props.getProperties2().equals(
				...
				// host = cis.getCoreModuleServiceProdProtocol() + "://" +
				// cis.getCoreModuleServiceProdDomain() + ":" +
				// cis.getCoreModuleServiceProdPort();
			...
			// store_id = "store1";
			// api_token = "yesguy";
			// order_id = "CSA-SHOPIZER30A";
			// cust_id = "CSA-SHOPIZER48A";
			// amount = "1.00";
			//String pan = "5454545454545454";//for testing
			//String expiry_date = "0612";//for testing
...
/*			// Get capturable transaction
			...
			if (trx == null) {
				// throw new Exception("No capturable transaction for orderid "
				// + order.getOrderId());
				TransactionException pe = new TransactionException(
			...
			if (!trx.getName().equals(PaymentConstants.PAYMENT_MONERIS)) {
				TransactionException pe = new TransactionException(
			...
			if (vo.getConfiguration("paymentmethod") == null
					|| !vo.getConfiguration("paymentmethod").equals(
							PaymentConstants.PAYMENT_MONERIS)) {
				// throw new
				// Exception("MonerisTransaction Payment Gateway not configured for moneris, cannot retreive credentials for merchantid "
				// + order.getMerchantid());
				TransactionException pe = new TransactionException(
						"MonerisTransaction Payment Gateway not configured for moneris, cannot retreive credentials for merchantid "
								+ order.getMerchantId());
				pe.setErrorcode("01");
				throw pe;
			}
			...
			// CentralIntegrationService cis =
			// ServicesUtil.getPaymentMetod(PaymentConstants.PAYMENT_MONERIS,
			// origincountryid);
			CoreModuleService cis = ModuleManagerImpl.getModuleServiceByCode(
					countrycode, PaymentConstants.PAYMENT_MONERIS, 1);// 1 is
																		// type
			...
			/*
			 * host = "esqa.moneris.com"; store_id = "store1"; api_token =
			 * "yesguy"; order_id = trx.getInternalGatewayOrderId(); amount =
...
/*
			...
			if (trx == null) {
				...
				// + order.getOrderId());
				TransactionException pe = new TransactionException(
						"No refundable transaction for orderid "
								+ order.getOrderId());
				pe.setErrorcode("01");
				throw pe;
			}
