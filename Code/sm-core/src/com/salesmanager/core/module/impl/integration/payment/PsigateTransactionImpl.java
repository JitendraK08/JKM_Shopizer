...
/*			PaymentService paymentservice = new PaymentService();
			...
			if (vo == null
					|| vo
							.getConfiguration(PaymentConstants.PAYMENT_PSIGATENAME) == null) {
				// throw new
				// Exception("Psigate Payment Gateway not configured for psigate, cannot retreive credentials for merchantid "
				// + order.getMerchantid());
				TransactionException pe = new TransactionException(
						"Psigate Payment Gateway not configured for psigate, cannot retreive credentials for merchantid "
								+ order.getMerchantId());
				pe.setErrorcode("01");
				throw pe;
			}
			...
			CoreModuleService cis = ModuleManagerImpl.getModuleServiceByCode(
					countrycode, PaymentConstants.PAYMENT_PSIGATENAME, 1);// 1
																			// is
																			// type
																			// credit
																			// card
...
*/
			...
			HttpClient client = new HttpClient();
			// client.getHostConfiguration().setHost(host,Integer.valueOf(port));
...
/*
			...
			if (trx == null) {
				// throw new Exception("No refundable transaction for orderid "
				// + order.getOrderId());
				TransactionException pe = new TransactionException(
			...
			// important, check if the transaction is made with the same gateway
			if (trx.getTransactionDetails() != null
					&& !trx.getTransactionDetails()
							.getMerchantPaymentGwMethod().equals(
									PaymentConstants.PAYMENT_PSIGATENAME)) {
