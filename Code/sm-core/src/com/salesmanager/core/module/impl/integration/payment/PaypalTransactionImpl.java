		...
		try {
			...
			if (strAck != null && strAck.equalsIgnoreCase("Success")) {
				...
				try {
					...
					// session =
					// HibernateUtil.getSalesManagerSessionFactory().openSession();
		...
		try {
			...
			String returnUrl = URLEncoder
				   ...
				   cancelUrl = URLEncoder
					.encode(
							ReferenceUtil.buildCheckoutUri(store)
									+ "/displayInvoice.action",
							"UTF-8"); //@TODO put in config file
...
/*			String currencyCodeType = order.getCurrency();
			...
			ConfigurationRequest configrequest = new ConfigurationRequest(order
					.getMerchantId(), PaymentConstants.MODULE_PAYMENT
					+ PaymentConstants.PAYMENT_PAYPALNAME);
			...
			IntegrationProperties ik = (IntegrationProperties) vo
					.getConfiguration("keys");

			if (ik == null) {
				TransactionException te = new TransactionException(
						"Paypal not configured for "
			...
			String countrycode = CountryUtil.getCountryIsoCodeById(store
					.getCountry());
			CoreModuleService serviceDefinition = ModuleManagerImpl
					.getModuleServiceByCode(countrycode,
							PaymentConstants.PAYMENT_PAYPALNAME, 0);*/
...
*/
			...
			// REQUEST
			// requiredSecurityParameters]&METHOD=RefundTransaction&AUTHORIZATIONID=01987219673867
	...
	public ConfigurationResponse getConfiguration(
		 ...
		 * StringTokenizer(configurations.getConfigurationValue2(),"|"); int
		 * i=1; Map names = new HashMap();
		 * 
		 * while(st.hasMoreTokens()) { String value = st.nextToken();//get
		 * 1;name StringTokenizer st2 = new StringTokenizer(value,";"); String l
		 * = null; String n = null; while(st2.hasMoreTokens()) { String v =
		 * st2.nextToken();
		 * 
		 * if(i==1) {//lang l = v; } if(i==2) {//name n = v; } i++; }
		 * names.put(l, n); i=1; } vo.addConfiguration("paypalnames", names);
		 */
