...
public class MerchantService {
	...
	public String getUser(HttpServletRequest request) throws MerchantException {
		...
		try {
			...
			// clz = Class.forName(conf.getString("core.logon.module.central"));

			// AdministrationLogonModule module =
			// (AdministrationLogonModule)clz.newInstance();
	...
	public void createNewOrSaveMerchant(MerchantStore creatorStore,
		...
		if (merchantUserInfo.getMerchantId() == null
				|| merchantUserInfo.getMerchantId().intValue() == 0) {
			// New Merchant User
			...
			String language = merchantUserInfo.getUserlang();
			...
			cservice.sendHtmlEmail(email, mailTitle, merchantUserInfo,
	...
	public void deleteMerchant(int merchantId) throws Exception {
		...
		MerchantUserInformation merchantUser = merchantUserInformationDao
				.findByMerchantId(merchantId);
		if (merchantUser != null) {
			merchantUserInformationDao.delete(merchantUser);
		}
		...
		// delete roles
		merchantUserRoleDao.deleteByUserName(merchantUser.getAdminName());
	...
	public List<MerchantStoreHeader> getAllMerchantStores() {
		...
		for (MerchantStore store : storeList) {
			...
			MerchantUserInformation userInfo = merchantUserInformationDao
					.findByMerchantId(store.getMerchantId());
			if (userInfo != null) {
				header.setAdminEmail(userInfo.getAdminEmail());
				header.setAdminName(userInfo.getAdminName());
			}
	 ...
	 */
	@Transactional
	public Collection<MerchantUserRole> getMerchantRoles(String userName)
			throws Exception {
		return merchantUserRoleDao.findByUserName(userName);
	}

	public MerchantUserInformation adminLogon(HttpServletRequest request)
	 ...
	 */
	@Transactional
	public MerchantUserInformation getMerchantUserInfo(int merchantId) {
