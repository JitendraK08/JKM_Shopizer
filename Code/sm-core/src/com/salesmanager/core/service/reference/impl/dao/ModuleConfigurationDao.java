...
public class ModuleConfigurationDao extends HibernateDaoSupport implements
	...
	public Collection<ModuleConfiguration> findByConfigurationModuleAndCountryCode(
		...
		try {
			...
			// List list =
			// super.getSession().createQuery("select c from ModuleConfiguration c where c.id.countryIsoCode2 in(:cId) and c.id.configurationModule=:mId")
			// .setParameterList("cId", countryList)
			// .setString("mId", configurationModule)
			// .list();
	...
	public Collection<ModuleConfiguration> findByConfigurationKeyAndCountryCode(
		...
		try {
			...
			// super.getSession().createQuery("select c from ModuleConfiguration c where c.id.countryIsoCode2 in(:cId) and c.id.configurationModule=:mId")
			// .setParameterList("cId", countryList)
			// .setString("mId", configurationKey)
			// .list();
