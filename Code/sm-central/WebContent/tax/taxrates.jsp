...
while(i.hasNext()) {
	...
	<tr bgcolor="<%=trcolor%>">
      ...
      <td valign="top"><input type="text" id="taxrate_<%=txsz%>" name="taxrate_<%=txsz%>" value="<%=CurrencyUtil.displayFormatedAmountNoCurrency(v.getTaxRate(),ctx.getCurrency()) %>"></td>
      <td valign="top"><input type="text" id="taxorder_<%=txsz%>" name="taxorder_<%=txsz%>" value="<%=v.getTaxPriority() %>"></td>
		...
		int langcount = 0;
	      ...
	      while(langiter .hasNext()) {
	      	...
	      	if(descriptions!=null) {
	      		...
	      		%>
	      		<tr><td><input type="text" id="taxlinedesc_<%=langcount%>_<%=txsz%>" name="taxlinedesc_<%=langcount%>_<%=txsz%>" value="<%=name %>">(<%=lang.getCode()%>)</td></tr>
	      		<%
	      	}
			langcount++;
