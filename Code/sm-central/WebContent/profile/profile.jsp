...
<head>
    ...
    <title><s:text name="label.menu.group.profile" /></title>
    <link type="text/css" rel="stylesheet" href="<%=request.getContextPath()%>/common/styles/main.css"/>
    <%if(SecurityUtil.isMerchantInRole(request,"admin")) {%>
...
<p>
                		<fieldset>
                		<legend>
                			<s:text name="label.menu.group.profile.information" />
                        </legend>
...
<%if(SecurityUtil.isMerchantInRole(request,"admin")) {%>
	<s:include value="profileleftmenu.jsp"/>
<%}%>
